/* ============================================================
   ...
   Stores: categories, coupons, settings, cart-side data.
   Products are served from the Node/Express backend.
   Orders, customers, reviews, newsletter are backend concerns.
   ============================================================ */
...
/* ============================================================
   DB.init — seeds essential reference data on first load
   ============================================================ */
const DB = {
  ...
  init() {
    ...
    if (ver !== DB_VERSION) {
      // Clear any stale data from a previous version
  ...
  _seed() {
    ...
    dbSet(DB_KEYS.settings, {
      ...
      phone:                  '+94 76 782 9707',
      address:                'Galle, Sri Lanka',
      currency:               'USD',
      currencySymbol:         '$',
      freeShippingThreshold:  75,
      defaultShippingCost:    9,
      whatsapp:               '94767829707',
    ...
    // Empty collections (real data lives on the backend)
  ...
  /* ============================================================
     PRODUCTS — fetched from server; localStorage not used
  ============================================================ */
  products: {
    // Kept for backward-compat with any admin page that calls DB.products.*
    // All real operations go through /api/products.php
    getAll()           { return []; },
    getActive()        { return []; },
    getById()          { return null; },
    getFeatured()      { return []; },
  ...
  orders: {
    ...
    getByCustomerEmail(e)  { return DB.orders.getAll().filter(o => o.customer.email === e); },
    add(data) {
  ...
  customers: {
    ...
    getByEmail(e)   { return DB.customers.getAll().find(c => c.email === e) || null; },
    add(data) {
    ...
    delete(id) { dbSet(DB_KEYS.customers, DB.customers.getAll().filter(c => c.id !== id)); },
    _upsert(customerData, orderTotal) {
      ...
      if (existing) {
        ...
        customers.push({
          id: uid(), name: customerData.name, email: customerData.email,
          country: customerData.country || '', totalOrders: 1, totalSpent: orderTotal, createdAt: now(),
        });
      }
      dbSet(DB_KEYS.customers, customers);
    },
  ...
  reviews: {
    ...
    getByProduct(pid)    { return DB.reviews.getApproved().filter(r => r.productId === pid); },
    avgRating(pid) {
      const r = DB.reviews.getByProduct(pid);
    ...
    delete(id) { dbSet(DB_KEYS.reviews, DB.reviews.getAll().filter(r => r.id !== id)); },
  },
  ...
  coupons: {
    ...
    add(data) {
      ...
      coupons.push(coupon); dbSet(DB_KEYS.coupons, coupons);
      return coupon;
    },
    update(id, data) {
      const coupons = DB.coupons.getAll();
    ...
    delete(id) { dbSet(DB_KEYS.coupons, DB.coupons.getAll().filter(c => c.id !== id)); },
  },
  ...
  newsletter: {
    ...
    subscribe(email, name = '') {
      ...
      subs.push(sub); dbSet(DB_KEYS.newsletter, subs);
      return { success: true };
    },
    unsubscribe(email) {
      const subs = DB.newsletter.getAll();
    ...
    delete(id) { dbSet(DB_KEYS.newsletter, DB.newsletter.getAll().filter(s => s.id !== id)); },
    exportCSV() {
  ...
  categories: {
    ...
    add(data) {
      ...
      cats.push(cat); dbSet(DB_KEYS.categories, cats);
      return cat;
    },
    update(id, data) {
      const cats = DB.categories.getAll();
    ...
    delete(id) { dbSet(DB_KEYS.categories, DB.categories.getAll().filter(c => c.id !== id)); },
  },
  ...
  settings: {
    ...
    update(data){ dbSet(DB_KEYS.settings, { ...DB.settings.get(), ...data }); },
  },
  ...
  blog: {
    ...
    add(data) {
      ...
      posts.unshift(post); dbSet(DB_KEYS.blog, posts);
      return post;
    },
    update(id, data) {
      const posts = DB.blog.getAll();
    ...
    delete(id) { dbSet(DB_KEYS.blog, DB.blog.getAll().filter(p => p.id !== id)); },
  },
  ...
  media: {
    ...
    add(data) {
      ...
      media.unshift(item); dbSet(DB_KEYS.media, media);
      return item;
    },
    delete(id) { dbSet(DB_KEYS.media, DB.media.getAll().filter(m => m.id !== id)); },
  },
